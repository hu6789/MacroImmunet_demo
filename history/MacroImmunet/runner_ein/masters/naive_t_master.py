...
NaiveTCellMaster
----------------
A lightweight Naive T cell master for the runner_ein demo.
...
- for nearby naive T cells, compute simple affinity (via env.compute_affinity) and emit intents:
  ...
  - percell_evaluate (if percell is enabled) -> lets percell decide (Th2 etc)
...
- small and focused so it plugs into your orchestrator_demo easily
- parameters configurable via params dict

This file intentionally contains a minimal BaseMaster class so it can be imported standalone
in runner_ein (you may elect to refactor to a shared base later).
...
class NaiveTCellMaster(BaseMaster):
    ...
    Params (defaults):
      ...
      - percell_type_map: dict mapping "CD4"->"Th2" etc (used to suggest percell type)
    ...
    def tick(self) -> List[Dict]:
        ...
        # emit tick event early (n_intents unknown yet)
        # we'll call emit_tick at end with count
        ...
        for c in naive_cells:
            ...
            if best_pm is None:
                # optionally still emit an activation event with zero affinity
                continue

            score = float(best_aff)

            # tcr_activation intent (informational)
            intents.append({
            ...
            if score >= self.affinity_threshold:
                ...
                # probability may be modified by local cytokines (e.g., IL12)
    ...
    def _collect_pmhc(self, coord: Tuple[int, int]) -> List[Dict]:
        ...
        # if antigen density > threshold, return a toy pmhc
        try:
            x, y = coord
            ag = 0.0
    ...
    def _best_affinity_for_cell(self, cell: Any, pmhcs: List[Dict]) -> Tuple[Optional[Dict], float]:
        ...
        for pm in pmhcs:
            ...
            if callable(comp):
                try:
                    # compute_affinity(pm, tcr) expected
                    # pick first TCR in repertoire if exists
    ...
    def _suggest_master_target(self, cell: Any, pm: Dict, affinity: float) -> str:
        # naive heuristic: CD4 -> Effector_Th1 or Effector_Th2 based on IL12/IL4 balance
    ...
    def _compute_differentiation_probability(self, coord: Tuple[int, int], cell: Any, pm: Dict, affinity: float) -> float:
        # base prob configured or default
        base = float(self.params.get("base_differentiation_prob", 0.2))
        # boost by affinity
        p = base + 0.6 * affinity
        # modulate by IL12/IL4 presence (IL12 boosts Th1 path probability)
